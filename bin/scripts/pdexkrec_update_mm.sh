...
HHSEARCHLOC=$PREFIX/software/hhsearch
...
do
  B=`basename $i .seq`;
  cp $i .;
  $HHSEARCHLOC/buildali.pl $B.seq -v $VERBOSE_MODE;
  $HHSEARCHLOC/hhmake -i $B.a3m -v $VERBOSE_MODE;
done;
cd $HERE;
...
if [[ -n "$(find $INPUT/negative_seq -name '*.seq' )" ]]; then
  cd $NEGATIVE_HHM;
...
cat /dev/null >$NEGATIVE_HHM_LIST
if [[ -n "$(find $NEGATIVE_HHM -name '*.hhm' )" ]]; then
...
do
  B=`basename $i .hhm`;
  $HHSEARCHLOC/hhsearch -i $i -d $PROFILE_DATABASE -o $B.out -ssm 4 -v $VERBOSE_MODE;
done;
cd $HERE;
