...
then
    ...
    then
	${prefix}/software/hhsearch/addpsipred.pl $INPUT_FILE -fas 1>> $LOG_FILE 2>> $ERR_FILE;
    else
	${prefix}/software/hhsearch/buildali.pl $INPUT_FILE -fas -v $VERBOSE_MODE 1>> $LOG_FILE 2>> $ERR_FILE;
    fi
else
...
then
    #Making profile
    ${prefix}/software/hhsearch/hhmake -i $MSA_FILE -v $VERBOSE_MODE 1>> $LOG_FILE 2>> $ERR_FILE
    #Converting final MSA to fasta
    ${prefix}/software/hhsearch/reformat.pl a3m fas $MSA_FILE $MSA_FASTA_FILE 1>> $LOG_FILE 2>> $ERR_FILE

    if [[ -e "$PRO_FILE" ]];
    then
	#Searching
	${prefix}/software/hhsearch/hhsearch -i $PRO_FILE -d $PRO_DATABASE -v $VERBOSE_MODE -o $OUT_FILE -ssm 4 1>> $LOG_FILE 2>> $ERR_FILE
	#Filtering
	${prefix}/software/hhsearch/hhfilter -i $MSA_FILE -o $FILT_A3M_FILE -id 90 -cov 0 -qid 0 -qsc -20.00 -diff 100 1>> $LOG_FILE 2>> $ERR_FILE
	#Converting to FASTA
	${prefix}/software/hhsearch/reformat.pl a3m fas $FILT_A3M_FILE $FILT_FAS_FILE 1>> $LOG_FILE 2>> $ERR_FILE
