...
use strict;
use lib "/home/coma/bin/software/hhsearch";

use Align;


# CUSTOMIZE
##my $dbbase="/usr/data/blast/db/nr"; # will use PSI-BLAST dbs $dbbase."90" and $dbbase."70" 
my $dbbase="/usr/data-blast/db/nr"; # will use PSI-BLAST dbs $dbbase."90" and $dbbase."70" 
                                   # It is recommended to use nre = nr + env. 
                                   # 90: filtered with CD-HIT to 90% maximum pairwise sequence identity

my $home = "/home/coma/bin";

my $hh = $home . "/software/hhsearch";

my $perl=$hh;
my $ncbidir= $home . "/software/blast-2.2.22/bin"; # directory with ncbi BLAST binaries
my $blastpgp= $home . "/software/blast-2.2.22/bin/blastpgp -I T -s T"; # blastpgp executable
our $blastpgp.= $home . "/software/blast-2.2.22/bin/blastpgp -I T -s T"; # show gi's in defline; use Smith-Waterman
my $execdir= $home . "/software/psipred261/bin";  # directory with PSIPRED binaries 
my $datadir= $home . "/software/psipred261/data"; # directory with PSIPRED data 
##my $dummydb= $home . "/software/dummydb"; # Name of a dummy blast database (single sequence formatted with formatdb) 
my $dummydb= "${dbbase}70"; # Name of a dummy blast database (single sequence formatted with formatdb) 
my $pdbdir= $home . "/data/input/pdb_structures";
