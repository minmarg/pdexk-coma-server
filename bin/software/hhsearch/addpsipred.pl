...
my $home = "/home/coma/bin/software";

my $ncbidir = "/usr/local/install/blast/bin";            # Put the directory path with the BLAST executables 
my $perl = $home . "/hhsearch";                 # Put the directory path where reformat.pl is lying
my $dummydb = $home . "/dummydb";               # Put the name given to the dummy blast directory (or leave this name)

my $psipreddir = $home . "/psipred261";     # Put the directory path with the PSIPRED executables 
...
# Use first sequence to define match states and reformat input file to a3m and psi
    &System("perl $perl/reformat.pl -v $v2 -M first $informat a3m $infile $inbase.inan");
        
# Read query sequence
    open (INFILE, "<$inbase.inan") or die ("ERROR: cannot open $inbase.inan: $!\n");
...
# If less than 26 match states => add sufficient number of Xs to the end of each sequence in $inbase.inan
...
# Reformat into PSI-BLAST readable file for jumpstarting 
    &System("perl $perl/reformat.pl -v $v2 -r -noss a3m psi $inbase.inan $inbase.psi");
...
# Remove temporary files
    ...
    system("rm $inbase.pn $inbase.sn $inbase.mn $inbase.chk $inbase.blalog $inbase.mtx $inbase.aux $inbase.ss $inbase.ss2 $inbase.sq");
