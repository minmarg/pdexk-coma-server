...
proc Main {} {
    ...
    #######################
    #To bypass the automatic detection of the several binaries please uncomment the next lines and replace it with your PATH
    
    set PATHMUSCLE [file join ~/software/aqua/muscle3.7/muscle]

    set PATHMAFFT [file join /usr/bin/mafft]
    #set PATHMAFFT [file join /g/bork6/muller/Software/mafft-6.611/bin/mafft]
    ...
    set env(MAFFT_BINARIES) [file join /usr/bin/]

    set PATHRASCAL [file join ~/software/aqua/rascal1.34/rascal]

    set PATHNORMD [file join ~/software/aqua/normd_noexpat/normd]
    ...
    #1-1 MUSCLE
    #
    #if you wish no to use MUSCLE uncomment next line
    #set NoMUSCLE 1
    set Message    ""
    if {$PATHMUSCLE==""} {
    ...
    #1-2 MAFFT
    #
    #if you wish no to use MAFFT uncomment next line
    #set NoMAFFT 1
    set Message   ""
    if {$PATHMAFFT==""} {
    ...
    foreach F $L_NORMD {
	if {![file exists $F]} {puts "One file normd does not exists yet ([file tail $F]), full stop";return ""}
    }

    #3- Selection within all MSA computed the best one
    set FileNORMD1 [lindex $L_NORMD 0]
    set FileNORMD2 [lindex $L_NORMD 1]

    set  BEST [Get_Best_NORMD $FileNORMD1 $FileNORMD2]
...
proc Get_Best_NORMD {File1 File2} {
    ...
    if {$BestFile==""} {set BestFile $File1}
