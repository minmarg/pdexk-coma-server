...
public class HibernateDataSource<IList> implements IDataSource
{
	...
	public PeriodicalWorkerParams getPeriodicalWorkerParams() 
	{
	    ...
	    {
	    	transaction = session.beginTransaction();
	    	List<PeriodicalWorkerParams> clusterList = session.createQuery("select o from " + 
	    								PERIODICAL_WORKER_TABLE + " as o").list();
	...
	public void initializeSystem()
	{
		...
		try
		{
			...
			if(getObjectNumber(session, USER_TABLE) == 0)
			{
				...
				ud.setPassword(passwordEncoder.encodePassword("admin",
